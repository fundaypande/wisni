<?php
  ...
  require_once("config.php");
   ...
   if(!isset($_GET['id'])){
     ...
     $query = $db->prepare("SELECT ttanaman.id_tanaman, ttanaman.nama AS 'tanaman', tpenyakit.nama, tpenyakit.keterangan FROM tpenyakit INNER JOIN ttanaman USING(id_tanaman) WHERE id_penyakit = :id");
     ...
     if(isset($_POST['submit'])){
         ...
         $query = $db->prepare("UPDATE `tpenyakit` SET `nama`=:nama,`keterangan`=:keterangan, `id_tanaman`=:tanaman WHERE `id_penyakit`=:id");
         ...
         if (!empty($result) ){
           ...
           echo 'alert("Tanaman Berhasil Diedit"); window.location.href="penyakit.php";';
           echo '</script>';
         }
     }
?>
